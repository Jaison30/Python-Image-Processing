...
if __name__ == '__main__':
    church = Image(filename='church.png')

    # existing
    church_existing = church
    church_existing.write_image('church.png')

    # brightening
    brightened_im = brighten(church, 1.7)
    brightened_im.write_image('church_brightened.png')

    # darkening
    darkened_im = brighten(church, 0.3)
    darkened_im.write_image('church_darkened.png')

    # increase contrast
    incr_contrast = adjust_contrast(church, 2, 0.5)
    incr_contrast.write_image('church_increased_contrast.png')

    # decrease contrast
    decr_contrast = adjust_contrast(church, 0.5, 0.5)
    decr_contrast.write_image('church_decreased_contrast.png')

    # blur using kernel 3
    blur_3 = blur(church, 3)
    blur_3.write_image('church_blur_k3.png')

    # blur using kernel size of 15
    blur_15 = blur(church, 15)
    blur_15.write_image('church_blur_k15.png')

    # let's apply a sobel edge detection kernel on the x and y axis
    sobel_x = apply_kernel(church, np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]))
    sobel_x.write_image('church_edge_x.png')
    sobel_y = apply_kernel(church, np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]]))
    sobel_y.write_image('church_edge_y.png')

    # let's combine these and make an edge detector!
    sobel_xy = combine_images(sobel_x, sobel_y)
    sobel_xy.write_image('church_edge_xy.png')
